...
export function createProviderSettingsController({
        ...
        state.providers = providers.map((provider) => {
            if (!isExtension && provider?.presetId) {
                ...
                return normalizeProviderConfig({
                    ...
                    presetId: null,
                    modelSource: 'manual',
                    userModels: provider.userModels?.length
                        ? provider.userModels
                        : (preset?.models?.[0] ? [{ ...preset.models[0], source: 'user' }] : []),
                });
            }
            return normalizeProviderConfig({
                ...provider,
    ...
    const renderHeader = (container) => {
        ...
        presetSelect.setAttribute('aria-label', tr('provider_preset_select', '选择 Provider 预设'));
        ...
        addButton.addEventListener('click', async () => {
            ...
            const provider = normalizeProviderConfig(preset ? {
                id,
                presetId: isExtension ? preset.id : null,
                name: preset.name,
                api: preset.api,
                ...
                userModels: isExtension || !preset.models[0]
                    ? []
                    : [{ ...preset.models[0], source: 'user' }],
            } : { ...defaultProvider(), id });
    ...
    const renderModelRow = (provider, model, list) => {
        ...
        radio.setAttribute('aria-label', `${tr('provider_select_model', '选择模型')} ${model.name || model.id}`);
    ...
    const renderModels = (provider, card) => {
        ...
        const count = element('div', 'provider-model-count');
        const renderFiltered = () => {
            ...
            matches.slice(0, 100).forEach((model) => renderModelRow(provider, model, list));
            count.textContent = matches.length > 100
                ? `${tr('provider_showing_models', '显示')} 100 / ${matches.length}`
                : `${matches.length} ${tr('provider_models_count', '个模型')}`;
        };
        ...
        section.append(search, count, list, addRow);
        card.append(section);
    };
